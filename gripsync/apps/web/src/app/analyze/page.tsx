          ...
          <div className="aspect-video bg-black relative flex items-center justify-center">
            ...
            <div className="absolute inset-0 border-2 border-dashed border-slate-800 m-8 rounded-xl flex flex-col items-center justify-center text-slate-600">
              ...
              <p className="font-medium text-lg">
                {step === 1 ? "Connect desktop client..." : 
                 step === 2 ? "Position hand in frame" : 
                 step === 3 ? "Play your favorite game" : 
                 "Analyzing data..."}
              </p>
            </div>
      ...
      <div className="flex justify-between items-center bg-surface p-6 rounded-xl border border-slate-800">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 text-sm text-accent">
            <div className="w-2 h-2 bg-accent rounded-full shadow-[0_0_8px_rgba(16,185,129,0.8)]"></div>
            Desktop Connected
          </div>
        </div>
        ...
        <div className="flex gap-4">
          {(step === 2 || step === 3) && (
            <Button 
              ...
              className="gap-2"
            >
              {isRecording ? <><Square className="w-4 h-4 fill-current" /> Stop Recording</> : <><Play className="w-4 h-4 fill-current" /> Start Recording</>}
            </Button>
          )}
