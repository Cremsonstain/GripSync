        ...
        <div className="absolute bottom-4 left-4 right-4 flex flex-wrap items-end justify-between gap-3">
          <div className="rounded-xl border border-slate-800 bg-slate-950/85 p-3 backdrop-blur">
            ...
            <div className="flex flex-wrap gap-1.5">
              {layouts.map((layout) => <button key={layout} type="button" aria-pressed={state.keyboardLayout === layout} onClick={() => setKeyboardLayout(layout)} className={`rounded-md px-2.5 py-1.5 text-xs font-medium transition ${state.keyboardLayout === layout ? 'bg-emerald-400 text-slate-950' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}>{layout}</button>)}
            </div>
          </div>
          <div className="rounded-xl border border-slate-800 bg-slate-950/85 p-3 backdrop-blur">
            <div className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">{labels.mousepad}</div>
            <div className="flex gap-1.5">
              {(['speed', 'control'] as const).map((surface) => <button key={surface} type="button" aria-pressed={state.mousepadSurface === surface} onClick={() => setMousepadSurface(surface)} className={`rounded-md px-3 py-1.5 text-xs font-medium transition ${state.mousepadSurface === surface ? 'bg-emerald-400 text-slate-950' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}>{surface === 'speed' ? labels.speed : labels.control}</button>)}
            </div>
          </div>
        </div>
      </div>
